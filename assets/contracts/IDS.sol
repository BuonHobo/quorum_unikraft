...
contract IDS {
    ...
    ) external onlyOwner lengthCheck(states.length, actions.length) {
    ...
    function proposeNewValues(
        ...
        for (uint256 i = 0; i < parameters.length; i++) {
            ...
            require(
                state_parameters_exist[parameter],
                string.concat("Parameter not Exist: ", parameter)
            );
