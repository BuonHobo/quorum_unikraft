...
function findImportsLoc(importPath) {
  try {
    const resolvedPath = path.resolve("./assets", importPath);
...
const compiledCode = JSON.parse(
  solc.compile(JSON.stringify(input), { import: findImportsLoc }),
);
...
async function deploy() {
  ...
  //const deployedAbiPath = path.join(__dirname, 'MyContractABI.json');
